...
class TestBloodworkImport:
    ...
    def test_import_creates_lab_report(self, tmp_path, db_session, sample_json):
        ...
        reports = db_session.exec(select(LabReport)).all()
        assert len(reports) == 1
        assert reports[0].lab_provider == "Quest"

    def test_import_creates_panels(self, tmp_path, db_session, sample_json):
        ...
        panels = db_session.exec(select(Panel)).all()
        assert len(panels) == 1
        assert panels[0].name == "Metabolic Panel"

    def test_import_creates_biomarkers(self, tmp_path, db_session, sample_json):
        ...
        biomarkers = db_session.exec(select(Biomarker)).all()
        assert len(biomarkers) == 2
    ...
    def test_import_sets_source_file(self, tmp_path, db_session, sample_json):
        ...
        report = db_session.exec(select(LabReport)).first()
        assert report.source_file == str(json_file)
    ...
    def test_import_no_partial_records_on_validation_error(self, tmp_path, db_client):
        ...
        with db_client.get_session() as session:
            reports = session.exec(select(LabReport)).all()
            panels = session.exec(select(Panel)).all()
            biomarkers = session.exec(select(Biomarker)).all()
